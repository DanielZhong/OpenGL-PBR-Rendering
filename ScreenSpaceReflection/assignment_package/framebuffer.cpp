...
const Texture& FrameBuffer::getTexture(GBufferOutputType a) {
    if(m_outputTextures.contains(a)) {
        return *m_outputTextures.at(a);
    }
...
void FrameBuffer::setDrawBuffers(const std::vector<GBufferOutputType> &attachmentTypes) {
    ...
    glContext->glDrawBuffers(attachments.size(), attachments.data());
}
...
CubeMapFrameBuffer::CubeMapFrameBuffer(QOpenGLFunctions_3_3_Core *context, unsigned int width, unsigned int height, unsigned int devicePixelRatio)
