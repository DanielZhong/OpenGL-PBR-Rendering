...
void Mesh::create(){
    //Count the number of vertices for each face so we can get a count for the entire mesh
        ...
        for(unsigned int i = 0; i < faces.size(); i++){
            ...
            vert_pos.push_back(tri.points[0]); vert_nor.push_back(tri.normals[0]);
            vert_pos.push_back(tri.points[1]); vert_nor.push_back(tri.normals[1]);
            vert_pos.push_back(tri.points[2]); vert_nor.push_back(tri.normals[2]);
